...
function StatsTab({ session }) {
  const [stats, setStats] = useState({
    ...
    cardSales: 0.0,
    upiSales: 0.0,
    totalSales: 0.0, // dailyRevenue fallback for split percentages
    invoiceCount: 0, // dailyCount fallback
  });
  const [loading, setLoading] = useState(true);
  ...
  const loadStats = async () => {
    try {
      const { data, error } = await supabase.from('orders').select('grand_total, payment_mode, created_at');
      if (error) throw error;
      ...
      setStats({
        ...
        cardSales,
        upiSales,
        totalSales: dailyRevenue, // overall daily revenue for split percentage computation
